/**
 * WC-MA-DPC-v1.0 — MPPT Data Parsing Contract
 * Generates .docx using WakeCap docx-generator base template
 * Content faithfully matches the reference PDF (WC-MA-DPC-v1.0.pdf)
 */
...
content.push(richBody([{ text: 'Current Implementation:', bold: true }]));
content.push(createBullet('MPPT Tag: 0x02', 0, V));
content.push(createBullet('MPPT Value Length: 27 bytes (1-byte bitmask + 26 bytes register data) = 0x1B', 0, V));
...
content.push(createSpecTable(
  ...
  [
    ...
    ['MPPT Controller', '0x02', '27', '0x1B', '1-byte bitmask + 26 bytes data']
  ], V
));
content.push(createBodyText('Parsing rule:', V));
content.push(createBullet('If Tag == 0x01: Parse as Weather Station (see companion document).', 0, V));
content.push(createBullet('If Tag == 0x02: Parse as MPPT (this document).', 0, V));
...
content.push(createHeading('3. Field Definitions (Tag 0x02 — MPPT, Length 0x1B)', 2, V));
content.push(createBodyText(
  'The following table defines the fields within the Value part of the TLV structure when Tag == 0x02 and Length == 0x1B. The first byte is the bitmask (Section 2.4). The remaining 26 bytes contain register data in the order shown.', V
));
...
content.push(createSpecTable(
  ...
  [
    ...
    ['R6', 'Bit 6', '24–25', '2', '0x9013', '0x03', 'Real-Time Clock']
  ], V
));
...
// R6
content.push(richBody([{ text: 'R6: Real-Time Clock (0x9013) — 2 bytes', bold: true }]));
content.push(createSpecTable(
  ['Offset', 'Bytes', 'Data Type', 'Unit', 'Description'],
  [['24–25', '2', 'Packed byte', 'MM:SS', 'MPPT device clock']], V
));
content.push(richBody([{ text: 'RTC Decoding:', bold: true }]));
content.push(...codeBlock([
...
content.push(createBullet('If Tag == 0x02 and Length == 0x1B (27): Parse as MPPT (Step 3).', 1, V));
...
content.push(richBody([{ text: 'Raw Hex (29 bytes):', bold: true }]));
content.push(...codeBlock([
...
content.push(createSpecTable(
  ...
  [
    ...
    ['R6 RTC', '0C 1E', 'Minutes=12, Seconds=30 → 12:30']
  ], V
));
...
content.push(createSpecTable(
  ...
  [
    ['Tag',              '0x01',                           '0x02'],
    ['Length',           '0x20 (32 bytes)',                '0x1B (27 bytes)'],
    ['Discriminator',   'Length == 32',                   'Length == 27'],
...
content.push(createBodyText('Complete byte-by-byte map of the 29-byte MPPT TLV packet:', V));
content.push(createSpecTable(
  ['Byte', 'Name', 'Field', 'Type', 'Notes'],
  [
    ['[0]',    'TAG',    'Tag',              'uint8',     'Always 0x02'],
    ['[1]',    'LEN',    'Length',           'uint8',     'Always 0x1B (27)'],
...
async function main() {
  ...
  const outPath = path.resolve(__dirname, 'WC-MA-DPC-v1.0.docx');
